...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  AccessDenied:
    "You declined the GitHub authorization. No worries — try again whenever.",
...
export default async function SignInPage({
  ...
  return (
    <div className="relative flex flex-1 flex-col items-center justify-center overflow-hidden bg-zinc-50 px-6 py-12 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <div
        aria-hidden
        className="hero-glow pointer-events-none absolute top-1/2 left-1/2 -z-10 h-[36rem] w-[36rem] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gradient-to-br from-emerald-400/25 via-sky-400/15 to-indigo-500/25 blur-3xl dark:from-emerald-500/15 dark:via-sky-500/10 dark:to-indigo-500/20"
      />

      <Link
        href="/"
        className="absolute top-6 left-6 inline-flex items-center gap-1.5 rounded-full border border-zinc-200 bg-white/70 px-3 py-1.5 text-sm font-medium text-zinc-600 backdrop-blur transition-all hover:bg-white hover:text-zinc-900 dark:border-zinc-800 dark:bg-zinc-900/70 dark:text-zinc-400 dark:hover:bg-zinc-900 dark:hover:text-zinc-100"
      >
        <svg
          width="14"
          height="14"
      ...
      <div className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white/80 p-8 shadow-xl shadow-zinc-900/5 backdrop-blur-md sm:p-10 dark:border-zinc-800 dark:bg-zinc-900/80 dark:shadow-black/20">
        ...
        <ul className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
          <Bullet>
            Read your profile, contributions, and repositories (public +
            private)
          </Bullet>
          <Bullet>
            We only ever read your data — never write or modify anything
          </Bullet>
